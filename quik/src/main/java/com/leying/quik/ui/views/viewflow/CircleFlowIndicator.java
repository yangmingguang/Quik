...
/**
 * 
* @ClassName: CircleFlowIndicator
* @Description: TODO(这里用一句话描述这个类的作用)
* @author YangMingGuang
* @date 2015-5-21 下午3:22:26
*
 */
public class CircleFlowIndicator extends View implements FlowIndicator, AnimationListener {
	...
	public CircleFlowIndicator(Context context) {
		...
		// set color
		//int color = SkinConfigManager.getInstance().getStringColor(context, SkinConstan.APP_THEME_COLOR);
		//initColors(color, 0xFFFFFFFF, STYLE_FILL, STYLE_STROKE);
	}
	...
	public CircleFlowIndicator(Context context, AttributeSet attrs) {
		...
		// Retrieve the radius
		radius = a.getDimension(R.styleable.CircleFlowIndicator_inactiveRadius, 8.0f);

		circleSeparation = a.getDimension(R.styleable.CircleFlowIndicator_circleSeparation, 5 * radius);
		...
		mCentered = a.getBoolean(R.styleable.CirclePageIndicator_centered, false);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int iLoop = 0; iLoop < count; iLoop++) {
			//			canvas.drawCircle(leftPadding + radius + (iLoop * circleSeparation) + centeringOffset, getPaddingTop()
			//					+ radius, radius, mPaintInactive);
			float left = leftPadding + (iLoop * circleSeparation) + centeringOffset;
			float top = getPaddingTop();
			float right = leftPadding + (iLoop * circleSeparation) + centeringOffset + 2 * radius;
			float bottom = getPaddingTop() + 2 * radius;
			canvas.drawRect(new Rect((int) left, (int) top, (int) right, (int) bottom), mPaintInactive);
		}
		float cx = 0;
		if (flowWidth != 0) {
			// Draw the filled circle according to the current scroll
			cx = (currentScroll * circleSeparation) / flowWidth;
			if (cx > (count - 1) * circleSeparation)
				cx = 0;
		}
		// The flow width has been upadated yet. Draw the default position
		//		canvas.drawCircle(leftPadding + radius + cx + centeringOffset, getPaddingTop() + radius, radius + activeRadius,
		//				mPaintActive);
		float left = leftPadding + cx + centeringOffset;
		float top = getPaddingTop();
		float right = leftPadding + cx + centeringOffset + 2 * radius;
		float bottom = getPaddingTop() + 2 * radius;
		canvas.drawRect(new Rect((int) left, (int) top, (int) right, (int) bottom), mPaintActive);
	}
